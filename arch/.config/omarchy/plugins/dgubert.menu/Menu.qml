  ...
  PanelWindow {
    ...
    BorderSurface {
      ...
      Item {
        ...
        // LOCAL CHANGE: Ctrl+K and Ctrl+J move the selection, and Ctrl+M acts
        // as Return. This is the only edit in this clone. See README.md.
        Keys.onPressed: function(event) {
          ...
          if (event.key === Qt.Key_Delete) {
            ...
            root.select(6)
            event.accepted = true
          } else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter || event.key === Qt.Key_Right || (event.key === Qt.Key_M && event.modifiers === Qt.ControlModifier)) {
            if (root.dmenuActive) {
