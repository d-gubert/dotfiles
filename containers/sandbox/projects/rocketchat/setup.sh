#!/usr/bin/env bash
#
# Runs IN THE SANDBOX from the generated kit's setup.install, once per create, as
# `agent`, with $DEVBOX_WORKSPACE as cwd — and BEHIND the egress allowlist, so
# every host it downloads from is listed in ./allowed-domains.
#
...
# dependencies. The toolchains land in $DEVBOX_TOOLS, a host directory shared by
# every workspace on this profile, so they are downloaded once per machine rather
# than once per checkout and never again on a rebuild.
#
...
# `volumes` declares them and the generated kit mounts them inside the workspace;
# a block volume's mount point lands root:root, so yarn and meteor — running as
# `agent` — would get EACCES creating anything under them.
#
# These target the *volumes*, not the host repo: each is mounted over the
# workspace mount at its path, so the host's own node_modules and .meteor/local
# are shadowed and untouched. Not recursive: everything below is created as
# agent, and a recursive walk of a populated node_modules would add minutes to
# every create.
log "claiming build volume mount points"
sudo chown agent:agent node_modules apps/meteor/.meteor/local
