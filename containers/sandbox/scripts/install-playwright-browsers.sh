#!/usr/bin/env bash
#
# Runs IN THE SANDBOX from the generated kit's setup.install, after the profile's
# setup.sh, once per create.
#
# Downloads the browser builds into the shared cache at
# $PLAYWRIGHT_BROWSERS_PATH — a host directory every workspace mounts, so the
# download happens once per machine rather than once per checkout. The template
# image installs the OS libraries those builds need; it deliberately does not
...
# match it to the `install-deps` line in ../Dockerfile).
set -euo pipefail

log() { printf '\033[1;34m[playwright]\033[0m %s\n' "$1"; }
...
# No --with-deps: that shells out to apt, which the allowlist does not cover, and
# the template image has already installed the same packages at build time. A
# no-op once the build is in the shared cache.
