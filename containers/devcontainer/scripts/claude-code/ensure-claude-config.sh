...
# every workspace is its own compose project, so an ordinary volume (or a
# devcontainer.json `mounts` entry, which becomes one) is namespaced per workspace
...
# `claude-code-config-${devcontainerId}` mount was: one volume, one login, every
# workspace on the machine — settings, history and skills included. Per-project
# state still stays apart, because `workspaceFolder` carries the checkout's full
# host path and Claude Code keys `~/.claude/projects/` by it.
#
# Idempotent: this runs on every create/rebuild in every workspace, and all but
# the first return at the check below.
set -euo pipefail

volume="devbox-claude-config"
...
log "created shared volume $volume — run \`claude\` and sign in once, in any workspace"
