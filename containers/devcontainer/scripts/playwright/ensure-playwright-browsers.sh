...
# read that one for the long version. In short: every workspace is its own compose
# project, so an ordinary volume (or a devcontainer.json `mounts` entry, which
# becomes one) is namespaced per workspace and each copy starts empty — meaning
# every workspace, and every rebuild, re-downloads ~500MB of browser builds.
# External also puts it out of reach of `docker compose down -v`.
#
# Sharing it across workspaces is safe even when they pin different Playwright
...
# each other. It is pure cache — `docker volume rm devbox-playwright-browsers`
...
volume="devbox-playwright-browsers"
...
log "created shared volume $volume — the first \`playwright install\` fills it, later ones in any workspace reuse it"
