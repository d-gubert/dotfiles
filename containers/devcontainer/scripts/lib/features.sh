...
#
# Sourced from both sides. On the host that is this directory in the dotfiles; in
# the container it is the same directory bind-mounted read-only at /opt/devbox,
# so devcontainer.json sits two levels up either way and the firewall can ask the
# same question about which features are on.
