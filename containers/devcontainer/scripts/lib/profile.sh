...
# container: its dependency install, the extra hosts its toolchain needs, the
# ports it serves on, the environment it expects. Everything in a profile is
# optional, and a workspace that matches nothing gets `default`, which is the
# bare toolchain image.
#
#   projects/<name>/
#     match              host path globs, one per line — what picks this profile
#     env                KEY=VALUE lines, added to the container's environment
#     env.local          the same, gitignored, for anything secret
#     ports              host:container per line, published by the container
#     allowed-domains    extra egress allowlist entries (domains or CIDRs)
#     setup.sh           runs in the container at updateContentCommand
#     compose/*.yml      raw compose fragments, for anything the above can't say
#
