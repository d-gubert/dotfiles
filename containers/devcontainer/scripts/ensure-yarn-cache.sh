...
# workspace is its own compose project, so an ordinary volume (or a
# devcontainer.json `mounts` entry, which becomes one) is namespaced per
# workspace and each copy starts empty — meaning every checkout re-downloads the
# same packages. External also puts it out of reach of `docker compose down -v`.
#
...
log "created shared volume $volume — the first \`yarn install\` fills it, later ones in any workspace reuse it"
